namespace NooseMod_LCPDFR.Callouts
{
    ...
    /// <summary>
    /// The robbery callout, also works as grand theft auto callout or simple pursuit.
    /// The use of this callout is to bypass NooseMod inactive mission when certain conditions there permits.
    /// Is active also when roaming.
    /// </summary>
    [CalloutInfo("NooseModPursuit", ECalloutProbability.Medium)]
    internal class Pursuit : Callout
    {
        ...
        /// </summary>
        private string[] vehicleModels, copcarModels;
        ...
        public Pursuit()
        {
            ...
            // Grab Game Episode for vehicle models
            if (Game.CurrentEpisode == GameEpisode.TLAD) vehicleModels = new string[] { "REGINA", "GBURRITO", "ADMIRAL", "SLAMVAN", "LYCAN", "FBI", "LANDSTALKER", "PATRIOT", "SABREGT", "WASHINGTON" };
            else if (Game.CurrentEpisode == GameEpisode.TBOGT) vehicleModels = new string[] { "APC", "BUFFALO", "F620", "LIMO2", "VADER", "CHAVOS", "REBLA", "PCJ", "TAXI", "STRETCH", "SULTAN" };
            else vehicleModels = new string[] { "SPEEDO", "BURRITO", "BURRITO2", "DF8", "FIRETRUK", "INTRUDER", "ZOMBIEB", "NRG900", "ORACLE" };

            if (Game.CurrentEpisode == GameEpisode.TBOGT) copcarModels = new string[] { "POLICE", "POLICE2", "POLICE3", "POLICE4" };
            else copcarModels = new string[] { "POLICE", "POLICE2" };
            ...
            if (this.vehicle.Exists())
            {
                ...
                for (int i = 0; i < this.robbers.Length; i++)
                {
                    ...
                    if (this.robbers[i].Exists())
                    {
                        ...
                        this.robbers[i].Weapons.RemoveAll();
                        this.robbers[i].Weapons.FromType(Weapon.Melee_Knife);
                        this.robbers[i].Weapons.DesertEagle.Ammo = Common.GetRandomValue(100, 999);
                        this.robbers[i].Weapons.Grenades.Ammo = Common.GetRandomValue(1, 4);
                        this.robbers[i].Weapons.DesertEagle.Select();
                ...
                // Create Police
                LVehicle copCar = new LVehicle(World.GetNextPositionOnStreet(this.vehicle.Position), Common.GetRandomCollectionValue<string>(this.copcarModels));
                if (copCar.Exists())
                {
